...
export const I18N = {
  ...
  about_api: {
    zh: `1、OpenAI 与 DeepSeek 均为 OpenAI 兼容接口，选择对应类型即可。`,
    en: `1. OpenAI and DeepSeek are both OpenAI-compatible interfaces; select the matching type.`,
    zh_TW: `1. OpenAI 與 DeepSeek 均為 OpenAI 相容介面，選擇對應類型即可。`,
    ja: `1. OpenAI と DeepSeek はどちらも OpenAI 互換インターフェースです。対応するタイプを選択してください。`,
    ko: `1. OpenAI와 DeepSeek는 모두 OpenAI 호환 인터페이스이므로 해당 유형을 선택하면 됩니다.`,
    tr: `1. OpenAI ve DeepSeek her ikisi de OpenAI uyumlu arayüzlerdir; uygun türü seçin.`,
  },
  about_api_2: {
  ...
  builtin_style_gallery: {
    zh: `内置样式画廊`,
    en: `Built-in Style Gallery`,
    zh_TW: `內建樣式畫廊`,
    ja: `内蔵スタイルギャラリー`,
    ko: `내장 스타일 갤러리`,
    tr: `Yerleşik Stil Galerisi`,
  },
  setting: {
    zh: `设置`,
    en: `Setting`,
  ...
  if_clear_cache: {
    zh: `翻译缓存策略`,
    en: `Translation cache policy`,
    zh_TW: `翻譯快取策略`,
    ja: `翻訳キャッシュポリシー`,
    ko: `번역 캐시 정책`,
    tr: `Çeviri önbelleği politikası`,
  },
  clear_cache_never: {
    zh: `保留 7 天，重启不清理`,
    en: `Keep for 7 days, no cleanup on restart`,
    zh_TW: `保留 7 天，重新啟動不清理`,
    ja: `7日間保持し、再起動時にクリアしない`,
    ko: `7일간 유지하고 재시작 시 정리하지 않음`,
    tr: `7 gün sakla, yeniden başlatmada temizleme`,
  },
  clear_cache_restart: {
    zh: `重启浏览器时清除缓存`,
  ...
  popup_rule_load_error: {
    zh: `暂时无法读取当前页面的翻译设置。请刷新当前页面后重试；如果仍然失败，可能是该页面不允许扩展运行。`,
    en: `Couldn't read the translation settings for this page. Refresh the page to retry; if it still fails, the page may not allow extensions.`,
    zh_TW: `暫時無法讀取目前頁面的翻譯設定。請重新整理目前頁面後重試；如果仍然失敗，可能是該頁面不允許擴充功能執行。`,
    ja: `このページの翻訳設定を読み込めませんでした。ページを更新して再試行してください。それでも失敗する場合は、このページで拡張機能が許可されていない可能性があります。`,
    ko: `이 페이지의 번역 설정을 읽을 수 없습니다. 페이지를 새로 고친 후 다시 시도하세요. 그래도 실패하면 이 페이지에서 확장 프로그램이 허용되지 않았을 수 있습니다.`,
    tr: `Bu sayfanın çeviri ayarları okunamadı. Sayfayı yenileyip tekrar deneyin; hâlâ başarısız olursa sayfa eklentilere izin vermiyor olabilir.`,
  },
  popup_section_page: {
    zh: `网页翻译`,
    en: `Page translation`,
  ...
  popup_section_language: {
    zh: `语言与服务`,
    en: `Languages & services`,
    zh_TW: `語言與服務`,
    ja: `言語とサービス`,
    ko: `언어 및 서비스`,
    tr: `Diller ve Hizmetler`,
  },
  popup_section_service: {
    zh: `服务与样式`,
    en: `Services & style`,
    zh_TW: `服務與樣式`,
    ja: `サービスとスタイル`,
    ko: `서비스 및 스타일`,
    tr: `Hizmetler ve Stil`,
  },
  popup_section_site: {
    zh: `本站规则`,
    en: `Site rules`,
  ...
  dictionary_failed: {
    zh: `词典查询失败`,
    en: `Dictionary lookup failed`,
    zh_TW: `辭典查詢失敗`,
    ja: `辞書の検索に失敗しました`,
    ko: `사전 검색에 실패했습니다`,
    tr: `Sözlük araması başarısız`,
  },
  dict_not_found: {
    zh: `未找到该词的释义`,
    en: `No definition found for this word`,
    zh_TW: `找不到該詞的釋義`,
    ja: `この単語の定義が見つかりません`,
    ko: `이 단어의 정의를 찾을 수 없습니다`,
    tr: `Bu kelime için tanım bulunamadı`,
  },
  suggestion_failed: {
    zh: `联想查询失败`,
    en: `Suggestion lookup failed`,
    zh_TW: `聯想查詢失敗`,
    ja: `サジェストの取得に失敗しました`,
    ko: `연관어 검색에 실패했습니다`,
    tr: `Öneri araması başarısız`,
  },
  sug_not_supported: {
    zh: `当前联想服务不可用`,
    en: `The current suggestion service is unavailable`,
    zh_TW: `目前聯想服務無法使用`,
    ja: `現在のサジェストサービスは利用できません`,
    ko: `현재 연관어 서비스를 사용할 수 없습니다`,
    tr: `Geçerli öneri hizmeti kullanılamıyor`,
  },
  zdic_not_found: {
    zh: `暂无字典数据，或查询失败`,
    en: `No dictionary data, or the lookup failed`,
    zh_TW: `暫無字典資料，或查詢失敗`,
    ja: `辞書データがないか、検索に失敗しました`,
    ko: `사전 데이터가 없거나 검색에 실패했습니다`,
    tr: `Sözlük verisi yok veya arama başarısız`,
  },
  empty_translation_result: {
    zh: `服务返回了空内容，请检查接口地址、密钥与提示词配置后重试。`,
    en: `The service returned an empty result. Check the endpoint, API key, and prompt settings, then retry.`,
    zh_TW: `服務回傳了空內容，請檢查介面位址、金鑰與提示詞設定後重試。`,
    ja: `サービスが空の結果を返しました。エンドポイント、APIキー、プロンプト設定を確認して再試行してください。`,
    ko: `서비스가 빈 결과를 반환했습니다. 엔드포인트, API 키, 프롬프트 설정을 확인한 후 다시 시도하세요.`,
    tr: `Servis boş bir sonuç döndürdü. Uç noktayı, API anahtarını ve istem ayarlarını kontrol edip tekrar deneyin.`,
  },
  capabilities: {
    zh: `能力`,
    en: `Capabilities`,
  ...
  touch_tap_7: {
    zh: `双指双击`,
    en: `Two-Finger Double Tap`,
    zh_TW: `雙指雙擊`,
    ja: `2本指ダブルタップ`,
    ko: `두 손가락 더블 탭`,
    tr: `Çift Parmak Çift Dokunma`,
  },
  translate_blacklist: {
    zh: `禁用翻译名单`,
  ...
  disabled_orilist: {
    zh: `跨域请求放行网站`,
    en: `Cross-origin allowed sites`,
    zh_TW: `跨域請求放行網站`,
    ja: `クロスオリジン許可サイト`,
    ko: `교차 출처 허용 사이트`,
    tr: `Çapraz kaynak isteğine izin verilen siteler`,
  },
  disabled_csplist: {
    zh: `放宽网页安全限制的网站`,
    en: `Sites with relaxed page security`,
    zh_TW: `放寬網頁安全限制的網站`,
    ja: `ページのセキュリティ制限を緩和するサイト`,
    ko: `페이지 보안 제한을 완화하는 사이트`,
    tr: `Sayfa güvenlik kısıtlamaları gevşetilen siteler`,
  },
  disabled_csplist_helper: {
    zh: `3、允许扩展向这些网页注入脚本、样式与媒体。除非你已了解相关风险，否则请谨慎使用。`,
    en: `3. Allows the extension to inject scripts, styles, and media into these pages. Use with caution unless you understand the related risks.`,
    zh_TW: `3. 允許擴充功能向這些網頁注入指令碼、樣式與媒體。除非你已了解相關風險，否則請謹慎使用。`,
    ja: `3. 拡張機能がこれらのページにスクリプト、スタイル、メディアを注入できるようにします。関連するリスクを理解していない限り、慎重に使用してください。`,
    ko: `3. 확장 프로그램이 이러한 페이지에 스크립트, 스타일, 미디어를 주입할 수 있도록 허용합니다. 관련 위험을 이해하지 못한 경우 주의해서 사용하세요.`,
    tr: `3. Uzantının bu sayfalara betik, stil ve medya enjekte etmesine izin verir. İlgili riskleri anlamadıkça dikkatli kullanın.`,
  },
  skip_langs: {
    zh: `不翻译的语言`,
  ...
  model_list_fetch_failed: {
    zh: `模型列表加载失败，请检查模型列表地址与密钥后重试`,
    en: `Failed to load the model list. Check the model list URL and API key, then retry.`,
    zh_TW: `模型列表載入失敗，請檢查模型列表位址與金鑰後重試`,
    ja: `モデル一覧の読み込みに失敗しました。モデル一覧URLとAPIキーを確認して再試行してください`,
    ko: `모델 목록을 불러오지 못했습니다. 모델 목록 URL과 API 키를 확인한 후 다시 시도하세요`,
    tr: `Model listesi yüklenemedi. Model listesi URL'sini ve API anahtarını kontrol edip tekrar deneyin`,
  },
  model_list_empty: {
    zh: `未获取到模型列表，可手动填写`,
  ...
  auto_trans_english: {
    zh: `英文自动翻译`,
    en: `Auto-Translate English`,
    zh_TW: `英文自動翻譯`,
    ja: `英語を自動翻訳`,
    ko: `영어 자동 번역`,
    tr: `İngilizceyi Otomatik Çevir`,
  },
  confirm_title: {
    zh: `确认`,
    en: `Confirm`,
