...
// 夸克等受限环境下，后台未捕获的 Promise 错误可能被关联到弹窗控制台；
// 这里统一抑制原生报错并输出日志，避免干扰 Popup 页面排查。
globalThis.addEventListener?.("unhandledrejection", (event) => {
  event?.preventDefault?.();
  const reason = event?.reason;
  const message =
    reason?.stack || reason?.message || String(reason || "unknown error");
  console.error("[background-unhandled]", message);
});
...
function getRegistrableDomain(input) {
  try {
    ...
    appLog("getRegistrableDomain error", err);
    return "";
  }
}
...
async function persistSeparateWindowBounds(bounds) {
  ...
  try {
    ...
    appLog("Final separate window bounds saved to storage", bounds);
  } catch (err) {
    appLog("Save separate window bounds error", err);
  }
}
  ...
  try {
    ...
    if (separateWindowId !== null) {
      ...
      if (existingWin) {
        ...
        appLog("Separate window is ready");
        return existingWin;
      }
    }
    ...
    appLog("open separate window error", err);
  }
}
...
async function addContextMenus(contextMenuType = 1) {
  try {
    ...
    appLog("remove contextMenus", err);
  }
...
async function getUiLanguage() {
  try {
    ...
    appLog("get UI language error", err);
    return "en";
  }
}
...
const messageHandlers = {
  ...
  [MSG_TRANS_GETRULE]: async () => {
    // Popup 的 content script 通道在夸克等受限环境下可能无响应，后台按当前标签页 URL 直接兜底计算规则。
    try {
      const tab = await getCurTab();
      if (!tab?.url) {
        return { error: "no active tab url" };
      }
      const setting = await getSettingWithDefault();
      const rule = await matchRule(tab.url);
      return { rule, setting };
    } catch (err) {
