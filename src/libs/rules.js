...
/**
 * 根据 href 匹配规则。
 * 合并顺序：个人规则 > 内置站点规则 > 全局规则。
 * @param {string} href 当前页面 URL
 * @returns {Promise<Object>} 合并后的最终规则
 */
export const matchRule = async (href) => {
  ...
  // 内置站点规则（Wiki、HN、X、YouTube、GitHub 等）作为次级兜底
  const matchedBuiltinRule = findMatchingRule(BUILTIN_RULES, href);

  // 如果没有匹配到任何规则，返回全局规则
  if (!matchedPersonalRule && !matchedBuiltinRule) {
    return globalRule;
  }

  // 合并规则：全局规则 <- 内置规则 <- 个人规则
  let finalRule = { ...globalRule };
  finalRule = mergeRules(finalRule, matchedBuiltinRule);
