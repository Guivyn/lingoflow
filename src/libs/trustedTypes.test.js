const POLICY_NAME = "lingoflow-policy";
...
describe("trustedTypesHelper", () => {
  ...
  test("reuses existing policy when duplicate policy error is thrown", () => {
    ...
    const createPolicy = jest.fn(() => {
      throw new Error("Policy lingoflow-policy already exists");
    });
  ...
  test("sanitizes HTML and does not retry when CSP blocks policy creation", () => {
    const createPolicy = jest.fn(() => {
      throw new Error(
        "Creating a TrustedTypePolicy named 'lingoflow-policy' violates the following Content Security policy directive: \"trusted-types gIqNx7 default\". The action has been blocked."
      );
    });
    globalThis.trustedTypes = {
      createPolicy,
  ...
  test("treats allow-duplicates CSP rejection as unavailable policy", () => {
    const createPolicy = jest.fn(() => {
      throw new Error(
        "Creating a TrustedTypePolicy named 'lingoflow-policy' violates the following Content Security policy directive: \"trusted-types fast-html dompurify 1DSScriptURL MeControlScriptURL @azure/ms-rest-js#xml.browser lit-html npsTrustedTypePolicy default 'allow-duplicates'\". The action has been blocked."
      );
    });
    globalThis.trustedTypes = {
      createPolicy,
