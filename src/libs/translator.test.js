...
describe("Translator rule styles", () => {
  ...
  test("keeps translated text when host style is not a CSSStyleDeclaration", async () => {
    ...
    expect(
      inner.querySelector(`.${Translator.LINGOFLOW_CLASS.retry}`)
    ).toBeNull();
  });
  ...
  test("adopts restored translation wrappers without retranslating", async () => {
    document.body.innerHTML = `
      <main id="root">
        <h3>
          ...
          <lingoflow class="lingoflow-wrapper notranslate">
            <font lang="zh-CN" class="lingoflow-inner">Existing translation</font>
          </lingoflow>
        </h3>
      </main>
    `;
    ...
    const requestedTexts = apiTranslate.mock.calls.map(([args]) => args.text);
    const inner = wrappers[0].querySelector(
      `.${Translator.LINGOFLOW_CLASS.inner}`
    );
    ...
    expect(
      document.querySelector(`h3 a .${Translator.LINGOFLOW_CLASS.warpper}`)
    ).toBeNull();
      ...
      <main id="root">
        <h3>
          <lingoflow class="lingoflow-wrapper notranslate">
            <br hidden>
            <font lang="zh-CN" class="lingoflow-inner">Existing translation</font>
            <template class="lingoflow-backup">
              <a href="/discussion/1">How to fix playback buttons?</a>
            </template>
          </lingoflow>
        </h3>
  ...
  test("does not query shadow roots inside LINGOFLOW translator elements when scanAll is enabled", async () => {
    document.body.innerHTML = `
      <main id="root">
        <div id="page-host">Page content</div>
        <div id="lingoflow-fab">
  ...
  test("injects inline <style> when CSSStyleSheet constructor is unavailable", async () => {
    ...
    expect(style.id).toBe("lingoflow-fallback-style");
