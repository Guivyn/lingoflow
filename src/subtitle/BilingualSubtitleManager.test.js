...
function getCaptionLines() {
  return Array.from(
    document.querySelectorAll(".lingoflow-caption-window p")
  ).map((node) => node.textContent);
}

function getCaptionBottom() {
  return parseFloat(
    document.querySelector(".lingoflow-caption-paper").style.bottom
  );
}
...
describe("BilingualSubtitleManager", () => {
  ...
  test("renders hover lookup word spans when enabled", () => {
    ...
    expect(
      Array.from(document.querySelectorAll(".lingoflow-subtitle-word")).map(
  ...
  test("updates current subtitle and list callback with streaming translation chunk", async () => {
    ...
    expect(
      document.querySelector(".lingoflow-caption-window").textContent
    ).toContain("部分译文");
  ...
  test("does not treat a click without movement as a drag position update", () => {
    ...
    manager.start();
    const handle = document.querySelector(".lingoflow-caption-window");
  ...
  test("treats dragged position as the hidden-controls bottom and floats after drag ends", async () => {
    ...
    manager.start();
    const paper = document.querySelector(".lingoflow-caption-paper");
    const container = document.querySelector(".lingoflow-caption-container");
    const handle = document.querySelector(".lingoflow-caption-window");
