...
/**
 * 管理 YouTube 播放器上的 LingoFlow 按钮、菜单和通知。
 */
export class YouTubePlayerUi {
  ...
  /**
   * 向 YouTube 右侧控制栏注入 LingoFlow 字幕菜单按钮。
   *
  ...
  injectToggleButton(ytControls) {
    if (
      ...
      !ytControls ||
      ytControls.querySelector(".lingoflow-subtitle-button")
    ) {
      return;
    }
  ...
  removeToggleButton() {
    ...
    const lingoflowControls =
      this.#toggleButton?.closest(".lingoflow-subtitle-controls") ||
      document.querySelector(".lingoflow-subtitle-controls");
    lingoflowControls?.remove();
