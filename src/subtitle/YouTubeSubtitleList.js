...
export class YouTubeSubtitleList {
  ...
  handleJumpMessage(event) {
    if (event.data && event.data.type === "LINGOFLOW_JUMP_TO_TIME") {
  ...
  downloadSubtitles() {
    ...
    try {
      ...
      downloadBlobFile(
        vttContent,
        `lingoflow-subtitles-${videoId}_${Date.now()}.vtt`
      );
    } catch (error) {
  ...
  downloadRawSubtitleEvents() {
    ...
    try {
      ...
      downloadBlobFile(
        jsonContent,
        `lingoflow-subtitles-raw-${videoId}_${Date.now()}.json`
      );
    } catch (error) {
  ...
  _renderTabsAndStructure() {
    ...
    tabHeader.append(subtitleTab, closeBtn);
    tabContentContainer.append(this.subtitleListEl);
  ...
  _createSubtitleListItem(sub, index) {
    ...
    li.id = `lingoflow-youtube-item-${index}`;
  ...
  updateSingleSubtitle(subtitleUpdate) {
    ...
    if (item) {
      const translationEl =
        item._translationEl ||
        item.querySelector(".lingoflow-youtube-translation");
      if (translationEl) {
  ...
  _downloadFile(content, mimeType, extension) {
    ...
    downloadBlobFile(
      blob,
      `lingoflow-vocabulary-${new Date().toISOString().slice(0, 10)}.${extension}`
    );
  }
