...
describe("YouTubeSubtitleList", () => {
    ...
    expect(
      Array.from(
        document.querySelectorAll(
          ".lingoflow-youtube-original .lingoflow-subtitle-word"
        )
  ...
  test("looks up hovered list words and records the subtitle start timestamp", async () => {
    ...
    document.addEventListener("lingoflow-add-word", addWordHandler);
    ...
    renderVisibleSubtitleItems(manager);
    document
      .querySelector(".lingoflow-subtitle-word")
    ...
    document.removeEventListener("lingoflow-add-word", addWordHandler);
    manager.destroy();
    jest.useRealTimers();
  });
  ...
  test("clears word tooltip when the subtitle list scrolls away from the hovered word", async () => {
    ...
    renderVisibleSubtitleItems(manager);
    const word = document.querySelector(".lingoflow-subtitle-word");
    ...
    expect(document.querySelector(".lingoflow-word-tooltip")).not.toBeNull();
    expect(word.classList.contains("lingoflow-word-hover")).toBe(true);

    manager.subtitleScrollContainer.dispatchEvent(new Event("scroll"));

    expect(document.querySelector(".lingoflow-word-tooltip")).toBeNull();
    expect(word.classList.contains("lingoflow-word-hover")).toBe(false);
  ...
  test("jumps only when clicking the time label", async () => {
    ...
    document.querySelector(".lingoflow-youtube-original").click();
    expect(videoEl.currentTime).toBe(0);

    document.querySelector(".lingoflow-youtube-item span").click();
